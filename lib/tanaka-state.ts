...
export function calcTanakaDelta(result: EvaluationResult): TanakaStatusDelta {
  ...
  // --- 精神衛生度（気持ち）---
  // 各軸を基準点(≒普通)からの差分で評価し、係数を大きめにして上下に振れさせる。
  let mental = 0;
  mental += (dg - 48) * 0.2; // 話を聞いてもらえた安心感（-9.6〜+10.4）
  mental += (sp - 48) * 0.16; // 支援・フォローの安心感（-7.7〜+8.3）
  mental += (pc - 50) * 0.05; // 何が問題か腑に落ちる（軽め）
  mental += (22 - h) * 0.18; // 穏やかさ↔威圧の基本影響（h22で±0）
  if (h >= 50) {
    mental -= (h - 50) * 0.5; // 人格否定・強い叱責は追加で大きく削る（-25）
  }

  // --- 意識改善レベル（納得・改善意欲）---
  // 「何が問題で何を直すか」の理解が中心。中身の薄い（低スコアの）指導では
  // しっかりマイナスに振れるよう、ボーナス加点を廃止して基準点50からの差で算出する。
  let awareness = 0;
  awareness += (pc - 50) * 0.25; // 問題点の明確さ（-12.5〜+12.5）
  awareness += (sp - 50) * 0.12; // 再発防止・次の行動（-6〜+6）
  awareness += (dg - 50) * 0.08; // 双方向のやり取り（-4〜+4）
  if (h >= 40) {
    awareness -= (h - 40) * 0.3; // 恐怖・萎縮で自発的な改善意欲が削がれる
  }
